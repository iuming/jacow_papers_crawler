            ...
            async with session.get(session_index_url) as response:
                if response.status == 200:
                    ...
                    for link in soup.find_all('a', href=True):
                        href = link['href']
                        if 'session' in href and 'index.html' in href and link.get_text(strip=True):
                    ...
                    for i, session_info in enumerate(session_links, 1):
                        ...
                        try:
                            async with session.get(session_info['url']) as session_response:
                                if session_response.status == 200:
                                    session_html = await session_response.text()
                                    papers = extract_papers_from_session(session_html, session_info['url'])
                                    ...
                                    print(f"   ❌ 无法访问 (状态码: {session_response.status})")
