...
class ConferenceDownloader:
    def __init__(self, conference_name, year, output_dir="data/papers"):
        ...
        # 创建输出目录
        self.paper_dir = (
            self.output_dir / self.year / self.conference_name / "individual_papers"
        )
    ...
    async def find_sessions(self):
        ...
        for link in soup.find_all("a", href=True):
            ...
            if (
                "-" in href
                and href.endswith("/index.html")
                and "/session/" not in href  # 排除递归链接
                and href != "index.html"
            ):
                    ...
                    if response.status == 200:
                        ...
                        if content_length:
                            ...
                            if size_mb > 100:  # 大于100MB
                                logger.warning(
                                    f"⚠️  文件过大 ({size_mb:.1f}MB)，跳过: {filename}"
                                )
                                return False
