...
class TestHelpers(unittest.TestCase):
    ...
    def test_sanitize_filename(self):
        """测试文件名清理"""
        # 测试非法字符
        self.assertEqual(sanitize_filename('test<>:"/\\|?*.pdf'), "test_________.pdf")
    ...
    def test_get_file_extension_from_url(self):
        ...
        # 直接URL
        self.assertEqual(
            get_file_extension_from_url("https://example.com/paper.pdf"), ".pdf"
        )
        ...
        self.assertEqual(
            get_file_extension_from_url(
                "https://example.com/download?filename=paper.pdf"
            ),
            ".pdf",
        )
...
class TestConfig(unittest.TestCase):
    ...
    def test_classification_keywords(self):
        ...
        self.assertIn(
            result, ["RF_Technology", "Accelerator_Technology"]
        )  # 两个都可能匹配
        ...
        # 测试默认分类
        self.assertEqual(
            self.config.classify_by_keywords("Random topic with no keywords"), "Other"
        )
    ...
    def test_get_conference_from_url(self):
        """测试从URL提取会议名"""
        self.assertEqual(
            self.config.get_conference_from_url("https://jacow.org/ipac2023/"), "IPAC"
        )
        self.assertEqual(
            self.config.get_conference_from_url("https://jacow.org/linac2022/"), "LINAC"
        )
        self.assertIsNone(
