...
class JACoWSpider:
    ...
    async def crawl_papers(self) -> List[Dict[str, str]]:
        ...
        async with self:
            ...
            # 解析会议链接
            conference_links = self._extract_conference_links_from_proceedings(proceedings_html)
            if self.logger:
            ...
            if self.conference_filter:
                conference_links = [
                    link for link in conference_links 
            ...
            for conf_link in conference_links:
                ...
                if self.logger:
                    self.logger.info(f"会议 {conf_link['name']} 找到 {len(conf_papers)} 篇论文")
    ...
    async def _fetch_page(self, url: str, retries: int = 3) -> Optional[str]:
        ...
        for attempt in range(retries):
            try:
                ...
                if self.logger:
                    self.logger.warning(f"请求失败 (尝试 {attempt + 1}/{retries}): {url} - {str(e)}")
    ...
    def _extract_conference_links_from_proceedings(self, html: str) -> List[Dict[str, str]]:
        """
        ...
        for link in soup.find_all('a', href=True):
            ...
            # 检查是否是会议链接
            if href and any(conf.lower() in href.lower() for conf in self.config.KNOWN_CONFERENCES):
                # 提取年份
    ...
    async def _crawl_conference_papers(self, conference_link: Dict[str, str]) -> List[Dict[str, str]]:
        """
        爬取单个会议的论文
        ...
        # 提取会议信息
        conference_name = conference_link.get('name', 'Unknown').split()[0]  # 取第一个词作为会议名
        year = conference_link.get('year') or extract_year_from_text(conference_link['name'])
        ...
        if not session_links:
            ...
            for session_url in session_links[:5]:  # 限制session数量以避免过多请求
                session_papers = await self._crawl_session_papers(session_url, conference_name, year)
    ...
    async def _crawl_session_papers(self, session_url: str, conference_name: str, year: int) -> List[Dict[str, str]]:
        """
        爬取单个session的论文
    ...
    def _extract_paper_links_from_session(self, soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
        """
    ...
    def _extract_direct_paper_links(self, soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
        """
        直接从页面提取PDF链接
            ...
            for next_elem in parent.next_siblings:
                if hasattr(next_elem, 'get_text'):
                    ...
                    if text and ',' in text and len(text) < 150:
                        # 排除一些明显不是作者的文本
                        if not any(word in text.lower() for word in ['doi:', 'paper:', 'slides:', 'received:', 'about:']):
                            return text
        
        return ''
    
    def _extract_paper_links(self, soup: BeautifulSoup, base_url: str) -> List[Dict[str, str]]:
        """
        从会议页面提取论文链接
        ...
        if not paper_links:
            ...
            for link in all_links:
                href = link.get('href')
                if href and any(re.search(pattern, href, re.I) for pattern in doc_patterns):
                    full_url = normalize_url(href, base_url)
                    if is_valid_url(full_url):
                        title = self._extract_paper_title(link)
                        authors = self._extract_authors(link)
        ...
        if parent:
            ...
            if author_elem:
                ...
                for sibling in parent.find_all(['p', 'span', 'div']):
                    ...
                    if (', ' in text and 
                        len(text) < 200 and 
                        not any(word in text.lower() for word in ['abstract', 'download', 'pdf'])):
                        authors = text
                        break
