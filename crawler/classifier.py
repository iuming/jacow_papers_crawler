            ...
            try:
                ...
                if year and year > 1980:  # 有效年份
                    target_dir = create_safe_directory(
                        self.base_dir, f"{conference}/{year}"
                    )
                else:
                    target_dir = create_safe_directory(
                        self.base_dir, f"{conference}/Unknown_Year"
                    )
                ...
                if self.logger:
                    self.logger.error(
                        f"组织论文失败: {paper.get('title', '')} - {str(e)}"
                    )
    ...
    def _organize_by_topic(self, papers: List[Dict[str, str]]):
        ...
        for paper in papers:
            try:
                ...
                # 使用配置中的分类方法
                topic = self.config.classify_by_keywords(
                ...
                if self.logger:
                    self.logger.error(
                        f"主题分类失败: {paper.get('title', '')} - {str(e)}"
                    )
                ...
                if conf_dir.is_dir() and conf_dir.name != "Topics":
                    ...
                    for year_dir in sorted(conf_dir.iterdir()):
                        if year_dir.is_dir():
                            ...
                            info_lines.append(
                                f"  📁 {year_dir.name}/ ({file_count} 个文件)"
                            )
            ...
            if topics_dir.exists() and topics_dir.is_dir():
                ...
                for topic_dir in sorted(topics_dir.iterdir()):
                    if topic_dir.is_dir():
                        ...
                        info_lines.append(
                            f"  📁 {topic_dir.name}/ ({file_count} 个文件)"
                        )
