...
class JACoWIndividualPaperSpider(JACoWSpider):
    ...
    async def crawl_individual_papers(
        self,
        ...
        if conference:
            conferences = [
                ...
                if conference.lower() in c.get("name", "").lower()
            ]
        ...
        try:
            ...
            if html:
                ...
                for link in soup.find_all("a", href=True):
                    ...
                    # 检查是否是会议链接
                    if href and (
                        "proceedings.jacow.org" in href or "jacow.org" in href
                    ):
                        # 提取年份
                        ...
                        if year_match:
                            ...
                            conferences.append(
                                {"name": text, "url": full_url, "year": year}
                            )
    ...
    async def _get_individual_papers_from_conference(
        self, conference_url: str
    ) -> List[Dict[str, str]]:
        """从单个会议获取所有单篇论文"""
        all_papers = []
        ...
        if sessions:
            ...
            for session in sessions:
                papers = await self._crawl_individual_papers_from_session(
                    session["url"]
                )
                all_papers.extend(papers)
        else:
    ...
    def _extract_session_links(self, html: str, base_url: str) -> List[Dict[str, str]]:
        ...
        for link in soup.find_all("a", href=True):
            ...
            if (
                href
                and ("session" in href.lower() or "/session/" in href)
                and "index.html" in href
                and text
            ):

                full_url = urljoin(base_url, href)
                sessions.append({"name": text, "url": full_url})

        return sessions

    async def _crawl_individual_papers_from_session(
    ...
    async def _crawl_individual_papers_from_page(
        self, page_url: str
    ) -> List[Dict[str, str]]:
        """从任意页面爬取单篇论文"""
    ...
    def _extract_paper_info_from_link(
    ...
    def _extract_paper_title_from_context(self, link: Tag, paper_code: str) -> str:
        ...
        for sibling in link.parent.find_next_siblings() if link.parent else []:
            ...
            if (
                text
                and len(text) > 10
                and not text.lower().startswith(("author", "doi", "cite"))
            ):
    ...
    def _clean_title(self, title: str) -> str:
        ...
        # 去掉常见的后缀
        title = re.sub(
            r"\s+(DOI:|Cite:|Author:|Abstract:).*", "", title, flags=re.IGNORECASE
        )
        # 去掉多余的空格
