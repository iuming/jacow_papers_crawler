...
class PaperDownloader:
    ...
    async def download_papers(
        self, papers: List[Dict[str, str]], resume: bool = False
    ) -> List[Dict[str, str]]:
        """
        下载论文列表
        ...
        async with self:
            ...
            # 执行下载任务
            if self.logger:
                progress = ProgressLogger(self.logger, len(papers), "下载进度")
    ...
    async def _download_single_paper(
        self, paper: Dict[str, str], resume: bool = False
    ) -> Dict[str, str]:
        """
        下载单个论文
    ...
    async def _do_download(
        ...
        try:
            ...
            if file_size_mb > self.max_size_mb:
                result["error"] = (
                    f"文件过大: {file_size_mb:.1f}MB > {self.max_size_mb}MB"
                )
    ...
    def _generate_filename(self, paper: Dict[str, str]) -> str:
        ...
        ).exists() or filename in self.downloaded_files:
            name, ext = (
