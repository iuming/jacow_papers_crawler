...
Usage Examples:
  ...
  python main.py --conference IPAC            # Only download IPAC conference papers
    ...
    parser.add_argument("--year", "-y", type=int, help="Specify year (e.g.: 2023)")
    ...
    parser.add_argument(
        "--resume", action="store_true", help="Resume download from last interruption"
    )
...
async def main():
    ...
    # Setup logging
    logger = setup_logger(
        name="jacow_crawler", log_file="data/logs/crawler.log", verbose=args.verbose
    )
    ...
    try:
        ...
        classifier = PaperClassifier(base_dir=output_dir, logger=logger)

        # Start crawling paper links
        logger.info("Starting to crawl paper links...")
        if args.individual_papers:
            paper_links = await spider.crawl_individual_papers(
                year=args.year, conference=args.conference, max_papers=args.max_papers
            )
        else:
        ...
        download_results = await downloader.download_papers(
            paper_links, resume=args.resume
        )
        ...
        await generate_report(
            download_results, classification_results, output_dir, logger
        )
...
async def generate_report(download_results, classification_results, output_dir, logger):
    ...
    try:
        ...
        total_size_mb = sum(
            r.get("size_mb", 0) for r in download_results if r.get("success", False)
        )
        ...
        logger.info(
            f"Successfully downloaded {successful_downloads}/{total_papers} papers, total size {total_size_mb:.2f} MB"
        )
